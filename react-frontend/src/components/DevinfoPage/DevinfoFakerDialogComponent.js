...
import devinfoFakerFactory from "./devinfoFakerFactory";
...
const DevinfoFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = devinfoFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("devinfo").create(elem));
...
export default connect(mapState, mapDispatch)(DevinfoFakerDialogComponent);
