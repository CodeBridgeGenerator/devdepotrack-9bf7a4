...
import DevinfoDatatable from "./DevinfoDataTable";
...
import DevinfoFakerDialogComponent from "./DevinfoFakerDialogComponent";
import DevinfoSeederDialogComponent from "./DevinfoSeederDialogComponent";
...
const DevinfoPage = (props) => {
    ...
    const filename = "devinfo.csv";

    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("devinfo");
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("devinfo").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Devinfo", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service("devinfo").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">Devinfo </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                    <Button label="add" loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="devinfo-add-button"/>
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="devinfo-datatable">
                    <DevinfoDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>
            ...
            <DevinfoEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <DevinfoCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <DevinfoFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <DevinfoSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(DevinfoPage);
